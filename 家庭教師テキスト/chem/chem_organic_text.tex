...
\section{芳香族炭化水素}
ベンゼン環の水素を炭化水素に置換したものを\textbf{芳香族炭化水素}という．ここらへんの化合物はあだ名で呼ばれることが多いので頑張って覚えよう．\\
\begin{minipage}{0.5\linewidth}
\begin{figure}[H]
    \centering
   \chemfig{*6(=-=(-CH3)-=-)}
...
\end{figure}
\end{minipage}
2つの置換基をもつ場合は，脂肪族炭化水素と同じように結合の位置を明示しないといけない．ベンゼンの場合は命名法が特殊なので気をつけよう．1つ目の置換基の位置を基準として，2つ目の置換基がどこにあるかで$o$(オルト)位，$m$(メタ)位，$p$(パラ)位の３つに分類される．\\
\begin{minipage}{0.33\linewidth}
\begin{figure}[H]
    \centering
   \chemfig{*6(=-=(-CH3)-(-CH3)=-)}
...
\newpage
\item[(2)]スルホン化\\
濃硫酸を加えて加熱すると水素がスルホ基\ce{-SO3H}で置換され，\textbf{ベンゼンスルホン酸}が生じる．スルホ化と呼ぶこともある．\\[5pt]
    \centerline{\chemfig{[:-30]*6(-=-=-=)}~$+$~\ce{H2SO4 -> }\chemfig{[:-30]*6(-=-(-SO_3H)=-=)}~$+$~\ce{H2O}}\\[5pt]
    \item[(3)]ニトロ化\\
硝酸と濃硫酸を加えて加熱すると水素がニトロ基\ce{-NO2}で置換され，\textbf{ニトロベンゼン}が生じる．\\[5pt]
...
\subsection{酸化反応}
ベンゼン環に結合した炭化水素基を酸化すると，長さによらずカルボキシ基になる．よって，一置換体の場合は必ず安息香酸が生成する．
\\[5pt]
